...
class GameGUI:
	def __init__(self, root):
		...
		self.play_speed = 500  # milliseconds between moves
		...
		style.configure('Controls.TLabel', background='#ecf0f1', font=('Helvetica', 12))  # Added for weight label
		...
		try:
			if os.path.exists(filename):
				with open(filename, 'r') as file:
					...
					self.current_state = GameState(map_data, weights)
					self.solver = None  # Reset solver for new puzzle
			...
			if player_pos:
				...
				if self.solver.can_move(self.current_state, y, x, dy, dx):
					...
					if target_cell in ['$', '*']:
						...
						stone_weight = self.current_state.stone_weights.get((new_y, new_x), 0)
		...
		for y, row in enumerate(state.map):
			for x, char in enumerate(row):
				...
				if char == '#':
					...
					if (y, x) in state.stone_weights:
						weight = state.stone_weights[(y, x)]
						text_x = (x1 + x2) / 2
						text_y = (y1 + y2) / 2
	...
	def auto_play(self):
		if self.is_playing:
			has_next = self.next_step()  # Returns True if there are more steps
			if has_next:
